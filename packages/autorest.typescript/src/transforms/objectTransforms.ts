...
export function transformObjects(
  ...
  const objectsWithAutoGeneratedSuffixMap: Map<string, ObjectDetails> = new Map<string, ObjectDetails>()
  objectDetails.filter(objectDetail => {
    return objectDetail.name.endsWith("AutoGenerated");
  }).map(obj => {
    objectsWithAutoGeneratedSuffixMap.set(obj.name, obj);
  });
  const objectsWithoutAutoGeneratedSuffix = objectDetails.filter(objectDetail => {
    if (objectsWithAutoGeneratedSuffixMap.has(objectDetail.name + "AutoGenerated")) {
      let foundTheSameSchema = false;
      const propertiesMap: Map<string, string> = new Map<string, string>();
      objectsWithAutoGeneratedSuffixMap.get(objectDetail.name + "AutoGenerated")?.properties.forEach(prop => {
        propertiesMap.set(prop.name, prop.type);
      })
      for (const prop of objectDetail.properties) {
        if (prop.name in propertiesMap.keys()) {
          if (prop.type === propertiesMap.get(prop.name)) {
            continue;
          } else {
            foundTheSameSchema = false;
          }
        } else {
          foundTheSameSchema = false;
        }
  ...
  console.log(objectsWithoutAutoGeneratedSuffix);
