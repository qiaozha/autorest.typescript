...
export function transformObjects(
  ...
  const objectsWithAutoGeneratedSuffixMap: Map<string, ObjectDetails> = new Map<
    string,
    ObjectDetails
  >();
  objectDetails
  ...
  const objectDetailsToKeep = objectDetails.filter(
    objectDetail => {
      if (
        objectsWithAutoGeneratedSuffixMap.has(
          objectDetail.name + "AutoGenerated"
        )
      ) {
        let foundTheSameSchema = true;
        const propertiesMap: Map<string, string> = new Map<string, string>();
        objectsWithAutoGeneratedSuffixMap
          .get(objectDetail.name + "AutoGenerated")
          ?.properties.forEach(prop => {
            propertiesMap.set(prop.name, prop.type);
          });
        if (objectDetail.properties.length !== propertiesMap.size) {
          return false;
        }
  ...
  return getObjectDetailsWithHierarchy(objectDetailsToKeep);
}
